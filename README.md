...
- `gco` - Checkout a branch or ref. If the branch is already checked out in another worktree, an interactive terminal offers to `cd` there via `gwtcd` (including when stdin is redirected but `/dev/tty` is available); non-interactive environments fail with git's usual error.
...
- `gwtcd [<fuzzy>]` — `cd` into a slot by fuzzy match against slot directory names **and** the branches currently checked out. With no argument, it switches to the primary checkout. Names match literally first (`tree-3`), then with `-`, `_`, and `/` ignored (`tree3`, `fixglass` → `ppr_fix-glass-…`), preferring the closest match when several normalize the same way. Literal branch substring hits stay ambiguous when several branches match; separator-insensitive branch matching applies when the query is not a literal substring. The primary checkout participates as the synthetic `root` slot, so the literal `root` **and** the branch it currently holds both route to the primary repo (e.g. `gwtcd main` when root has `main` checked out). Ambiguous matches print the candidate list and fail; no match prints what is available and fails.
